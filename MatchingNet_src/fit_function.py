...
from MatchingNet_src.utils import pairwise_distances
...
def matching_net_episode(model: Module,
    ...
    :param model: Matching Network to be trained.
    :param optimiser: Optimiser to calculate gradient step from loss
    :param loss_fn: Loss function to calculate between predictions and outputs
    :param x: Input samples of few shot classification task
    :param y: Input labels of few shot classification task
    :param n_shot: Number of examples per class in the support set
    :param k_way: Number of classes in the few shot classification task
    :param q_queries: Number of examples per class in the query set
    :param distance: Distance metric to use when calculating distance between support and query set samples
    :param fce: Whether or not to us fully conditional embeddings
    :param train: Whether (True) or not (False) to perform a parameter update
    :return:
...
def matching_net_predictions(attention: torch.Tensor, n: int, k: int, q: int) -> torch.Tensor:
    ...
    :param attention: torch.Tensor containing softmax over query-support distances.
        Should be of shape (q * k, k * n)
    :param n: Number of support set samples per class, n-shot
    :param k: Number of classes in the episode, k-way
    :param q: Number of query samples per-class
    :return:
