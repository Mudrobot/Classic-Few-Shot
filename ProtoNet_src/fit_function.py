'''
This file is written by user to define the behavior of an episode of training
'''
import torch
...
from ProtoNet_src.utils import pairwise_distances
...
def compute_prototypes(support: torch.Tensor, k: int, n: int) -> torch.Tensor:
    """
    Compute class prototypes from support samples.(k-way n-shot)
    :param support: torch.Tensor. Tensor of shape (n * k, d) where d is the embedding dimension.
    :param k: int. "k-way" i.e. number of classes in the classification task
    :param n: int. "n-shot" of the classification task
    :return: class_prototypes: Prototypes aka mean embeddings for each class
    """
